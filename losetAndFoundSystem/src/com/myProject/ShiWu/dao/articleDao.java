...
import com.myProject.ShiWu.entity.*;
	public class articleDao extends ParentDao 
	{
/*** 获得最大ID** @return*/
		public int GetMaxId()
		{
		...
		{
		 sql = "insert into article( Title,Author,Clicks,Contents,AddTime,Remark) values (?,?,?,?,?,?)";
			paramsList = new ArrayList<Object>();
						paramsList.add(model.getTitle());
			paramsList.add(model.getAuthor());
			paramsList.add(model.getClicks());
		...
		{
		 sql = " update  article set  Title=?,Author=?,Clicks=?,Contents=?,AddTime=?,Remark=? where Id=? ";
			paramsList = new ArrayList<Object>();
						paramsList.add(model.getTitle());
			paramsList.add(model.getAuthor());
			paramsList.add(model.getClicks());
		...
		public article GetModel(int id)
		{
			...
			Map row = res.getRows()[0];
					...
					model.setAuthor((String)row.get("Author"));
					model.setClicks((Integer)row.get("Clicks"));
		...
		/***得到列表** @return*/
		public List<article> GetList(String strWhere)
		{
			...
			if (res != null && res.getRowCount() >0) {// 存在数据
			 list = new ArrayList<article>();//开始循环
		...
		public List<article> GetListByPage(int pageSize, int currentPage,String strWhere)
		{
			if(strWhere.equals("")) {
					strWhere = " 1=1 ";
			}
			List<article> list = null;
			sql = "select top " + pageSize + " * from article " ;
			String sw = " where " + strWhere + " order by id Desc ";
			sql +=" where " + strWhere ;
			if (currentPage > 1) {
					sql += " and id not in(select top "+ (pageSize * (currentPage - 1)) + " id from article "+ sw + " ) ";
			}
					sql += " order by id Desc ";
			Result res = bd.getResultSet(sql, paramsList);
			if (res != null && res.getRowCount() >0) {// 存在数据
		...
		public List<article> GetListByPage2(int pageSize, int currentPage,String strWhere)
		{
			if(strWhere.equals("")) {
					strWhere = " 1=1 ";
			}
		...
		{
			List<article> list = this.GetList(strWhere);
			int totalCount = 0;
