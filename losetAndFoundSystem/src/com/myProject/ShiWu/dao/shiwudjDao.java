...
import com.myProject.ShiWu.entity.*;
	public class shiwudjDao extends ParentDao 
	{
/*** 获得最大ID** @return*/
		public int GetMaxId()
		{
			sql = "select max(id) from shiwudj";
		...
		/***添加** @return*/
		public int  Add(shiwudj model)
		{
		 sql = "insert into shiwudj( Title,WuPin,Times,Address,WaiXing,OtherBiaoShi,Phone,UserId,BaoChou,Contents,AddTime,Remark) values (?,?,?,?,?,?,?,?,?,?,?,?)";
			...
			paramsList.add(model.getWaiXing());
			paramsList.add(model.getOtherBiaoShi());
			paramsList.add(model.getPhone());
			paramsList.add(model.getUserId());
			paramsList.add(model.getBaoChou());
		...
		/***修改** @return*/
		public int Update(shiwudj model)
		{
		 sql = " update  shiwudj set  Title=?,WuPin=?,Times=?,Address=?,WaiXing=?,OtherBiaoShi=?,Phone=?,UserId=?,BaoChou=?,Contents=?,AddTime=?,Remark=? where Id=? ";
			paramsList = new ArrayList<Object>();
						paramsList.add(model.getTitle());
			paramsList.add(model.getWuPin());
			paramsList.add(model.getTimes());
		...
		{
			sql ="delete from shiwudj where id="+id;
			result = bd.getAffectRow(sql, paramsList);
			return result;
		}

		/***得到实体** @return*/
		public shiwudj GetModel(int id)
		{
			 sql="select * from shiwudj where id="+id;
			shiwudj model=null;
			...
			Map row = res.getRows()[0];
					...
					model.setWaiXing((String)row.get("WaiXing"));
					model.setOtherBiaoShi((String)row.get("OtherBiaoShi"));
					model.setPhone((String)row.get("Phone"));
					model.setUserId((Integer)row.get("UserId"));
					model.setBaoChou((String)row.get("BaoChou"));
		...
		/***得到列表** @return*/
		public List<shiwudj> GetList(String strWhere)
		{
			List<shiwudj> list = null;
			sql = "select * from shiwudj " ;
			if (!strWhere.equals("")) {
					sql += " where " + strWhere;
			}
			...
			if (res != null && res.getRowCount() >0) {// 存在数据
			 list = new ArrayList<shiwudj>();//开始循环
			 for (int i = 0; i < res.getRowCount(); i++) {
			// Result的每一行数据以key—value的形式保存（即Map）
			Map row = res.getRows()[i];
			shiwudj model = new shiwudj();
					model.setId((Integer)row.get("Id"));
					model.setTitle((String)row.get("Title"));
					model.setWuPin((String)row.get("WuPin"));
					model.setTimes((String)row.get("Times"));
		...
		public List<shiwudj> GetListByPage(int pageSize, int currentPage,String strWhere)
		{
			if(strWhere.equals("")) {
					strWhere = " 1=1 ";
			}
			List<shiwudj> list = null;
			sql = "select top " + pageSize + " * from shiwudj " ;
			String sw = " where " + strWhere + " order by id Desc ";
			sql +=" where " + strWhere ;
			if (currentPage > 1) {
					sql += " and id not in(select top "+ (pageSize * (currentPage - 1)) + " id from shiwudj "+ sw + " ) ";
			}
					sql += " order by id Desc ";
			Result res = bd.getResultSet(sql, paramsList);
			if (res != null && res.getRowCount() >0) {// 存在数据
		...
		public List<shiwudj> GetListByPage2(int pageSize, int currentPage,String strWhere)
		{
			if(strWhere.equals("")) {
					strWhere = " 1=1 ";
			}
		...
		{
			List<shiwudj> list = this.GetList(strWhere);
			int totalCount = 0;
