...
$(document).ready(function() {
	...
	<body>
		<form name="form1" action="renling.do" method="post">
			<div class="place">
				<span>当前位置:</span>
				<ul class="placeul">
					<li>
				...
				<table class="tablelist">
					...
					<tbody>
						<%
							...
							String sw = " 1=1 ";
							List<renling> list = dal.GetListByPage2(pageSize, currentPage, sw);
							if (list != null && list.size() > 0) {
								for (int i = 0; i < list.size(); i++) {
									renling model = list.get(i);
						%>
						<tr>
								<td><%=new shiwuzlDao().GetModel(model.getShiWuZLId())
							.getTitle()%></td>

							<td><%=model.getUserS()%></td>
...
function isDelete(objId) {
	if (confirm("您确定要删除吗？")) {
		window.location.href = "../../renling.do?action=delete&id=" + objId;
	}
}
</script>
